...
const Learn = () => {
    ...
    const isLogical = courseType === 'logical';
    const isVerbal = courseType === 'verbal';
    const courseData = isLogical ? logicalCourseData : (isVerbal ? verbalCourseData : aptitudeCourseData);
    const pageTitle = isLogical ? "Logical Reasoning Masterclass" : (isVerbal ? "Verbal Ability Masterclass" : "Quantitative Aptitude Masterclass");

    // Set default active video to the first video of the first topic
    const [activeVideo, setActiveVideo] = useState(courseData[0]?.videos[0] || null);
    const [activeTopicId, setActiveTopicId] = useState(courseData[0]?.topic || "");
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-orange-500/30">
            {/* TOP NAVIGATION BAR */}
            <div className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/5 py-4 px-6 md:px-10 flex items-center justify-between">
            ...
            <div className="max-w-[1700px] mx-auto p-4 md:p-6 lg:padding-8 flex flex-col lg:flex-row gap-6 lg:gap-8 min-h-[calc(100vh-80px)]">
                    ...
                    {activeVideo ? (
                        <>
                            ...
                            <div className="mt-6 flex-shrink-0">
                                <div className="inline-flex items-center gap-2 px-3 py-1 bg-orange-500/10 border border-orange-500/20 text-orange-400 rounded-lg text-[10px] font-black uppercase tracking-widest mb-3">
                                ...
                                <p className="text-sm font-semibold text-neutral-400 flex items-center gap-2">
                                    From Module: <span className="text-white">{activeTopicId}</span>
                                </p>
                            </div>
                        </>
                    ) : (
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full group flex items-start gap-3 p-3 rounded-xl transition-all text-left ${
                                                    isPlaying 
                                                    ? "bg-orange-500/10 border border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.1)]" 
                                                    : "hover:bg-white/5 border border-transparent"
                                                }`}
                                            >
                                                <div className="mt-0.5">
                                                    {isPlaying ? (
                                                        <div className="w-5 h-5 rounded-full bg-orange-400 flex items-center justify-center animate-pulse shadow-[0_0_10px_rgba(249,115,22,0.8)]">
                                                            <PlayIcon className="w-3 h-3 text-black ml-0.5" />
                                                        </div>
                                                    ) : (
                                                        <div className="w-5 h-5 flex items-center justify-center font-bold text-[10px] text-neutral-500 group-hover:text-amber-500 transition-colors">
                                                            {idx + 1}.
                                                        </div>
                                                    )}
                                                </div>
