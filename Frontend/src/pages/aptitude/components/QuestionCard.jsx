...
const QuestionCard = ({ question, index }) => {
  ...
  // 🎨 Color palette for tags (adjusted to ZenCode theme)
  ...
  return (
    <Link
      ...
      className="group glass-panel p-4 rounded-[20px] border border-white/5 hover:border-orange-500/40 hover:shadow-[0_0_15px_rgba(249,115,22,0.15)] transition-all flex items-center justify-between"
    >
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            ...
            {/* Category */}
            <span
              className={`badge text-[10px] font-bold uppercase tracking-wide px-2 py-2.5 rounded-lg ${getColorForString(
            ...
            {/* Topic */}
            <span
              className={`badge text-[11px] font-semibold px-2 py-2.5 rounded-lg ${getColorForString(
            ...
            {/* Difficulty */}
            <span
              className={`badge text-[10px] font-bold px-2 py-2.5 rounded-lg uppercase ${difficultyColors[question.difficulty]}`}
            >
              {question.difficulty}
            </span>
            ...
            {question.company && (
              <span
                className={`badge text-[10px] font-bold uppercase px-2 py-2.5 rounded-lg ${getColorForString(
                  question.company
                )}`}
              >
                {question.company}
