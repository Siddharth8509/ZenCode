...
const PDF_ISSUE_MESSAGES = {
  cloudinaryAcl:
    'This PDF is blocked by your Cloudinary account settings. Enable public delivery for PDF/ZIP files in Cloudinary Security settings, then re-upload if needed.',
  unavailable:
...
const PdfViewer = () => {
  ...
  useEffect(() => {
    ...
    setActivePdf((currentPdf) => {
      ...
      const updatedSelection = pdfs.find((pdf) => pdf._id === currentPdf._id);
      return updatedSelection || pdfs[0];
    });
  }, [pdfs]);
  ...
  const fetchPdfs = async () => {
    try {
      setLoading(true);
      const { data } = await axiosClient.get('/learning/api/pdfs');
      setPdfs(data.pdfs || []);
    } catch (error) {
      console.error('Failed to load learning PDFs:', error);
  ...
  const handleDownload = async (pdf) => {
    ...
    try {
      ...
      link.href = blobUrl;
      link.download = `${pdf.title || 'study-material'}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(blobUrl);
    } catch (error) {
      console.error('PDF download fallback error:', error);
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-black text-white selection:bg-blue-500/30">
      <div className="sticky top-16 z-40 bg-black/80 backdrop-blur-md border-b border-white/5 py-4 px-6 md:px-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-neutral-500">View and download PDFs</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white border border-white/10 rounded-2xl overflow-hidden md:h-[calc(100vh-160px)]">
          {activePdf ? (
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-black/10 bg-neutral-950 px-5 py-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-blue-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-neutral-950/80 backdrop-blur-sm">
                    <div className="glass-panel rounded-2xl border border-white/10 px-6 py-4 text-center">
                      ...
                      <p className="mt-2 text-xs text-neutral-400">
                        If the preview takes too long, use "Open in new tab".
                      </p>
                    </div>
                  </div>
                )}
                ...
                {viewerError && (
                  <div className="absolute inset-0 flex items-center justify-center bg-neutral-950 px-6">
                    <div className="max-w-md rounded-2xl border border-white/10 bg-black/60 p-6 text-center">
                      <DocumentArrowDownIcon className="mx-auto mb-4 h-12 w-12 text-blue-400" />
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-neutral-900 border border-white/10 text-neutral-500 p-8 text-center">
              ...
              <p className="text-lg font-bold text-white mb-2">No Document Selected</p>
              <p className="text-sm">Please select a PDF document from the list on the right.</p>
            </div>
          )}
        </div>
        ...
        <div className="w-full lg:w-[450px] flex-shrink-0 flex flex-col h-[600px] lg:h-[calc(100vh-160px)] glass-panel rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar bg-black/20 p-4 space-y-3">
            {loading ? (
              ...
              <div className="text-center text-neutral-500 p-8">No PDFs available yet.</div>
            ) : (
              pdfs.map((pdf) => (
                <button
                  key={pdf._id}
                  ...
                  <div
                    className={`p-2 rounded-lg ${
                      activePdf?._id === pdf._id
                        ...
                        : 'bg-white/5 text-neutral-400 group-hover:text-blue-400'
                    }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4
                      className={`font-bold truncate ${
                        activePdf?._id === pdf._id ? 'text-blue-400' : 'text-neutral-200 group-hover:text-white'
                      }`}
                    >
                      {pdf.title}
                    </h4>
                    <div className="mt-2 flex items-center justify-between gap-3">
                      <p className="text-[11px] font-bold text-neutral-500 uppercase tracking-wider bg-white/5 inline-block px-2 py-0.5 rounded">
                        {pdf.subject}
                      </p>
